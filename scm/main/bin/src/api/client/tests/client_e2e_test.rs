// Colocated tests for the `Client` trait contract (api/client/traits/client.rs)
// and its `ClientError` (api/client/errors/client_error.rs), exercised
// through a test-double implementation, per SEA's api/ test-organization
// rules. The real `BrowseClient` implementation is exercised end-to-end
// (spawning the actual compiled binary) by cli_e2e_test.rs and
// help_int_test.rs instead — this file verifies the *contract* itself, not
// `browse`'s specific subcommand behavior.
//
// Wrapped in `#[cfg(test)] mod tests { ... }` (rather than bare top-level
// items) so the whole body — including the test-double's `impl Client for
// FakeClient` and its `#[test] fn`s — is recognized as test code by tooling
// that strips `#[cfg(test)]` sections, the same convention already used by
// every other test module in this crate (e.g. core/session/store.rs).
